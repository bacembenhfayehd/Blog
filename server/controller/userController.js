...
const changeAvatar = async (req, res, next) => {
    ...
    if (avatar.size > 500000) {
        return next (new HttpError('Picture is too big. Should be less than 50kb',422))
    }

    let fileName;
    fileName = avatar.name;
    let splittedFilname = fileName.split('.')
    let newFileName = splittedFilname[0] + uuid() + '.' + splittedFilname[splittedFilname - 1]
...
const  editUser = async (req, res, next) => {
    try {
        ...
        // we want to update other details with/without changing email ( wich mean a unique id because we use it to login)
